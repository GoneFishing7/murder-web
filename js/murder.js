$(document).ready(function () {
  ...
  var roles = {};
  var rolesArr = [];
  var mode = "default"; // Can be default or oneSpec or noSheriff
  ...
  function validateForm() {
    if (numPlayers < MIN) {
      ...
      message(
        "Danger",
        "Oops!",
        "Please enter a # of players bigger than (or equal to) " + MIN + ". 🤓"
      );
      return false;
      ...
      message(
        "Danger",
        "Oops!",
        "Please enter a # of players less than " + MAX + ". 😊"
      );
      return false;
    ...
    for (const role in roles) {
      if (roles.hasOwnProperty(role)) {
        roles[role] =
          mode == "oneSpec"
            ? 1
            : Math.floor(numPlayers / POSSIBLE_ROLES.length);
        // Make sure that innocent has the oveflow
        if (role == "Innocent") {
          roles[role] =
            mode == "oneSpec" // If the mode is one special:
  ...
  $(".enter-player-names").on("show.bs.dropdown", function () {
    ...
    if ($names.children().length < $players.val()) {
      while ($names.children().length < $players.val()) {
        $names.append(
          `<input type='text' class='dropdown-item border name' ` +
  ...
  $("#reveal-btn").on("click", function () {
    if (currentRole < numPlayers) {
      ...
      if (!revealed) {
        // If we haven't revealed it yet
        $(".reveal-msg").empty(); // Get rid of the old reveal message

        // Add a reveal message
        $(".reveal-msg").html(
          "Your role is <span class='role' id='" +
            rolesArr[currentRole].toLowerCase() +
            "'>" +
            rolesArr[currentRole] +
            "</span>!"
        );
        ...
        $("#reveal-lbl").html(
          "☝<br>click this once you've read and memorized it. (Then, let the next person get their role!)" +
            "<br>" +
            (rolesArr[currentRole] == "Innocent" ? "" : othersMessage)
        );
  ...
  function genOthersMessage(role, others) {
    ...
    if (others.length > 2) {
      for (let index = 0; index < others.length - 1; index++) {
        ...
        othersMessage +=
          (!names[currentPerson]
            ? `Person #${currentPerson + 1}`
            : names[currentPerson]) + ", ";
      }
      othersMessage += "and ";
    } else if (others.length == 2) {
      othersMessage +=
        (!names[others[0]] ? "Person #" + (others[0] + 1) : names[others[0]]) +
        " and ";
    }
    if (others.length >= 1) {
      othersMessage =
      ...
      othersMessage +=
        (!names[others[others.length - 1]]
          ? "Person #" + (others[others.length - 1] + 1)
          : names[others[others.length - 1]]) + "</span>.";
    }
